...
public class AuthUtil {
    ...
    public String determineProviderIdFromOAuth2User(OAuth2User oAuth2User, String registrationId) {
        String providerId = switch (registrationId.toLowerCase()) {
            ...
            case "facebook" -> oAuth2User.getAttribute("id").toString();  // Facebook gives "id"
            case "twitter" -> oAuth2User.getAttribute("id").toString();   // Twitter also gives "id"
            case "email" -> oAuth2User.getAttribute("email").toString();  // If logging in via email-based provider
            default -> {
